...
<div class="bg-white rounded-lg shadow-md">
    <div class="flex border-b">
        @if(auth()->user()->role == 'superadmin')
            ...
            <a href="{{ route('superadmin.users.index', ['role' => 'superadmin']) }}" class="px-6 py-3 {{ $role == 'superadmin' ? 'border-b-2 border-orange-500 font-bold' : 'text-gray-500' }}">Admin Utama</a>
        @else
            ...
            @forelse($users as $user)
                <tr class="border-b hover:bg-gray-50">
                    ...
                    <td class="p-4 flex items-center space-x-2">
                        <a href="{{ auth()->user()->role == 'superadmin' ? route('superadmin.users.edit', $user) : route('admin.users.edit', $user) }}" class="text-blue-500 p-2 rounded-full hover:bg-blue-100">
                        ...
                        <form action="{{ auth()->user()->role == 'superadmin' ? route('superadmin.users.destroy', $user) : route('admin.users.destroy', $user) }}" method="POST" onsubmit="return confirm('Apakah Anda yakin ingin menghapus akun ini?');">
                            @csrf
                            @method('DELETE')
                            <button type="submit" class="text-red-500 p-2 rounded-full hover:bg-red-100">
