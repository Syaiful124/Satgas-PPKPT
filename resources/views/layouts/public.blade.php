...
<body class="bg-gray-100">
    <header class="text-black shadow-md w-full">
        <nav class="container mx-auto flex justify-between items-center">
            <a href="{{ route('beranda') }}" class="font-bold text-xl w-auto">
                SATGAS PPKPT STIMATA
            </a>
            <div class="hidden md:flex items-center space-x-4 w-auto">
                <a href="{{ route('pengaduan.create') }}" class="bg-gray-600 hover:text-orange-200 px-4 py-2 rounded-md">
                ...
                @else
                    <a href="{{ route('login') }}" class="bg-white text-orange-500 px-4 py-2 rounded-md hover:bg-gray-200">
                    ...
                    <a href="{{ route('register') }}" class="bg-blue-500 px-4 py-2 rounded-md hover:bg-blue-600">
                        Register</a>
                @endauth
            </div>
            <div class="md:hidden">
         ...
         <div id="mobile-menu" class="hidden md:hidden px-6 pt-2 pb-4 space-y-2">
             <a href="{{ route('pengaduan.create') }}" class="block hover:text-orange-200">Kirim Pengaduan</a>
             <a href="{{ route('unduhan') }}" class="block hover:text-orange-200">Unduhan</a>
             <a href="{{ route('hubungi.kami') }}" class="block hover:text-orange-200">Hubungi Kami</a>
             <hr class="my-2 border-orange-400">
             @auth
                <a href="{{ route('account.index') }}" class="block hover:text-orange-200">Account</a>
                <form action="{{ route('logout') }}" method="POST">@csrf<button type="submit" class="w-full text-left">Logout</button></form>
             @else
                <a href="{{ route('login') }}" class="block hover:text-orange-200">Login</a>
                <a href="{{ route('register') }}" class="block hover:text-orange-200">Register</a>
            @endauth
        </div>
    </header>
